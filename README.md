...
- Requirements listed in the header comment【F:spec.py†L1-L6】.
- Acquisition loop that averages several spectra, applies boxcar smoothing
  and finally stores the result as tab‑separated values【F:spec.py†L49-L67】.
...
- Docstring describing its purpose and how to run it【F:speclive.py†L1-L8】.
- Acquisition routine which averages several readings and updates the plot
  accordingly【F:speclive.py†L60-L76】.
...
- Introductory documentation that lists its dependencies【F:speclive2.py†L1-L5】.
- Use of `QtWidgets.QShortcut` to toggle acquisition with the space key
  while the timer runs at 10 Hz【F:speclive2.py†L41-L55】.
...
- The beginning of the script provides a utility that converts wavelengths
  into RGB values for the strip image【F:speclive3.py†L15-L36】.
- The `update_frame` method builds this coloured line from the latest
  intensities and displays it under the graph【F:speclive3.py†L88-L110】.
...
- The docstring enumerates the available hot‑keys, including saving the data
  and screenshots【F:speclive4.py†L1-L9】.
- During startup a cross‑hair cursor is created and mouse movements update the
  displayed wavelength and intensity in the status bar【F:speclive4.py†L50-L116】.
- Functions `save_csv`, `save_png` and `save_all` allow exporting the current
  ...
  with both CSV and PNG files【F:speclive4.py†L120-L152】.
